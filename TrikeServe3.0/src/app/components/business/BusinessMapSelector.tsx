...
interface BusinessMapSelectorProps {
  onClose: () => void;
...
export default function BusinessMapSelector({ onClose, onSelectLocation, currentAddress }: BusinessMapSelectorProps) {
  ...
  const reverseGeocode = useCallback(async (lat: number, lng: number) => {
    ...
    try {
      ...
      console.warn("[BusinessMapSelector] Reverse geocode failed:", err);
    }
    return null;
  }, []);

  // When the business taps on the map, drop a pin and look up the address
  const handleMapClick = useCallback(
    async (lat: number, lng: number) => {
      ...
      // Ignore stale responses if the business tapped somewhere else meanwhile
  ...
  useEffect(() => {
    ...
    if (!GOOGLE_MAPS_API_KEY) {
      // No key configured: let the business know search isn't available
      setSearchResults([]);
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        console.warn("[BusinessMapSelector] Autocomplete failed:", err);
        setSearchResults([]);
        setShowSearchDropdown(true);
      } finally {
  ...
  const handleSearchResultPick = async (suggestion: any) => {
    ...
    try {
      ...
      console.warn("[BusinessMapSelector] Place details failed:", err);
    } finally {
  ...
  const renderMap = () => {
    ...
    return (
      ...
      >
        {/* Pin the business placed (click or search) */}
        {pickedPin && (
          <MarkerF
