...
`src/`) and the React/Vite frontend app at `TrikeServe3.0/TrikeServe3.0/` — the frontend
...
2. Install dependencies with npm (the frontend app is npm-based — `package-lock.json`):
       ...
       cd TrikeServe3.0/TrikeServe3.0
...
- The portable node dir **must be on PATH** before starting: npm's child `vite.cmd`
  shim falls back to a bare `node` lookup and fails with `'node' is not recognized`
  otherwise. This is why the command prepends it to `$env:Path`.
